...
/gcalint reminder — Set reminder offset for an integration
/gcalint reminders — Toggle reminders on/off for an integration
"""
...
REDIRECT_URI  = "https://soren.retrac.ca/gcal/callback"
...
class GcalIntSetupModal(discord.ui.Modal):
    def __init__(self, guild_id: int, schedule: str, post_day: str,
        ...
        self.add_item(discord.ui.InputText(
            ...
            placeholder="e.g. TNC Events, MMA Schedule",
            max_length=50,
        ))
        self.add_item(discord.ui.InputText(
            label="Channel ID",
        ...
        await interaction.response.send_message(
            embed=discord.Embed(
                ...
                description=(
                    ...
                    "2. After authorizing, you'll be taken to a Soren page showing your authorization code.\n"
                    "3. Copy the code from that page.\n"
                    "4. Run `/gcalint verify` here in Discord and paste the code in.\n\n"
                    f"[Click here to authorize with Google]({auth_url})"
                ),
...
class GcalIntegrations(commands.Cog):
    ...
    async def gcalint_add(self, ctx: discord.ApplicationContext):
        ...
        if count >= limit:
            description = (
                f"{'Premium' if premium else 'Free'} servers can connect up to "
                f"**{limit} calendar{'s' if limit != 1 else ''}** ({count}/{limit} used).\n\n"
                + (
                    "Please remove an existing integration to add a new one."
                    if premium else
                    f"Upgrade to **[Soren Premium](https://soren.retrac.ca)** for up to {PREMIUM_GCAL_LIMIT} integrations."
                )
            )
            await ctx.respond(
                embed=discord.Embed(
        ...
        if not GCAL_AVAILABLE:
            await ctx.respond(
                embed=discord.Embed(
                    description="❌ Google Calendar libraries are not installed.",
                    color=discord.Color.red(),
                ),
                ephemeral=True,
            )
        ...
        if not os.path.exists(CREDS_FILE):
            await ctx.respond(
                embed=discord.Embed(
                    description=f"❌ Missing `{CREDS_FILE}`.",
                    color=discord.Color.red(),
                ),
                ephemeral=True,
            )
    ...
    async def gcalint_verify(
        ...
        code: discord.Option(str, "The authorization code from the Soren callback page."),
    ):
        if not await _require_admin(ctx):
            return
        ...
        if not pending:
            await ctx.respond(
                embed=discord.Embed(
                    description="❌ No pending connection found. Run `/gcalint add` first.",
                    color=discord.Color.red(),
                ),
                ephemeral=True,
            )
        ...
        await ctx.respond(
            embed=discord.Embed(
                ...
                description="Authorization successful! Select which calendar to connect.\n\n⭐ = Primary calendar",
                color=discord.Color.blurple(),
            ),
    ...
    async def gcalint_reminder(
        ...
        with get_connection() as conn:
            conn.execute(
                "UPDATE gcal_integrations SET reminder_offset=? WHERE id=?",
                (offset_int, integration_id),
            )
            conn.commit()
    ...
    async def gcalint_reminders(
        ...
        with get_connection() as conn:
            conn.execute(
                "UPDATE gcal_integrations SET reminders_enabled=? WHERE id=?",
                (new_val, integration_id),
            )
            conn.commit()
    ...
    async def reminder_loop(self):
        ...
        fetches upcoming events and fires a reminder embed if:
        ...
        for row in rows:
            ...
            # For short offsets, skip the API call during the "dead zone" of
            # the hour where no reminder could plausibly need to fire.
            if offset < 60:
                ...
                if dead_zone_start < dead_zone_end and dead_zone_start <= current_minute <= dead_zone_end:
                    log.debug(
                        f"gcalint reminder: skipping API call for integration {int_id} "
                        f"(minute {current_minute} in dead zone {dead_zone_start}–{dead_zone_end})"
                    )
                    continue
            ...
            for event in events:
                ...
                if not channel:
                    log.warning(
                        f"gcalint reminder: channel {integration['channel_id']} "
                        f"not found for integration {int_id}"
                    )
                    continue
                ...
                try:
                    ...
                    log.warning(
                        f"gcalint reminder: no permission in channel {integration['channel_id']}"
                    )
                except Exception as e:
    ...
    def _is_due(self, integration: dict, now: datetime) -> bool:
        ...
        if schedule == "daily":
            ...
            return (
                now.strftime("%A").lower() == integration.get("post_day", "monday")
